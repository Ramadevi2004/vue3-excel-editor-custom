  ...
  <div ref="editor" class="vue-excel-editor" :style="{display: 'inline-block', width}">
    ...
    <ul ref="autocomplete" v-show="focused && autocompleteInputs.length" class="autocomplete-results" @mousedown.prevent.stop @keydown.exact.esc="hideMultiSelectDropdown">
      ...
      <li v-for="(item, i) in autocompleteInputs"
          :key="i"
          :class="{select: autocompleteSelect === i}"
          @mousedown.left.prevent="inputAutocompleteText(item, $event)"
          class="autocomplete-result">{{ item }}</li>
    </template>
    </ul>
  </div>

      <!-- <ul ref="autocomplete" v-show="focused && autocompleteInputs.length" class="autocomplete-results" @click.stop @mousedown.stop>
    <template v-if="currentField && currentField.type === 'multiselect'">
      <li v-for="(item, i) in autocompleteInputs"
          :key="i"
          class="autocomplete-result multiselect-item"
          @click.stop="toggleMultiselectItem(item)">
      ...
      <li class="multiselect-actions">
        <button class="apply-btn" 
                @click.stop.prevent="applyMultiselectChoices"
                @mousedown.stop.prevent>Apply</button>
        <button class="clear-btn" 
                @click.stop.prevent="clearMultiselectChoices"
                @mousedown.stop.prevent>Clear</button>
      </li>
    </template>
    <template v-else>
      <li v-for="(item, i) in autocompleteInputs"
...
export default defineComponent({
  ...
  computed: {
    ...
    columnFilterString() {
      Object.keys(this.columnFilter).forEach((key) => {
        const val = this.columnFilter[key]

        if (Array.isArray(val)) {
          // Remove if all items are empty strings or whitespace
          this.columnFilter[key] = val.filter(v => String(v).trim() !== '')
          if (this.columnFilter[key].length === 0) {
          ...
          if (val.trim() === '') {
            ...
            this.columnFilter[key] = val.trim()
          }
        } else {
          // If it's neither string nor array (e.g., null or object), remove it
          delete this.columnFilter[key]
        }
      })
  ...
  watch: {
    'currentField': {
    handler(newField) {
      if (newField && newField.type === 'multiselect') {
        this.$nextTick(() => {
  ...
  methods: {
    ...
    */
        ...
        const doList = async () => {
          ...
          if (field.options) {
            ...
            if (field.options.constructor.name.endsWith('Function')) {
              ...
              // Filter out already selected values for multiselect
              if (field.type === 'multiselect' && this.inputBox.value) {
                const delimiter = field.delimiter || ','
                const selectedValues = this.inputBox.value.split(delimiter)
                  .map(v => v.trim())
                  .filter(v => v)
                  .map(v => v.toUpperCase())
    ...
    isItemSelected(item) {
      if (!this.currentField || !this.inputBox || this.currentField.type !== 'multiselect') return false;
      
      const delimiter = this.currentField.delimiter || ',';
      const currentValue = this.inputBox.value || '';
      
      // More direct comparison using includes for better matching
      return currentValue.split(delimiter)
    ...
    toggleMultiselectItem(item) {
      ...
      const existingIndex = selectedItems.findIndex(selectedItem => 
        selectedItem.localeCompare(item, undefined, { sensitivity: 'base' }) === 0
      );
      ...
      if (this.$refs.input) {
       ...
       this.$nextTick(() => {
        // Force-refresh autocomplete list without closing
        this.calAutocompleteList(true);
      });
    },

    
    applyMultiselectChoices() {
      if (e) {
    e.preventDefault();
    e.stopPropagation();
  }
      ...
      // Write the current value to the cell
      this.inputCellWrite(this.inputBox.value);
    ...
    handleMultiselectItemClick(text, e) {
  if (e) {
    e.preventDefault();
    e.stopPropagation();
  }
  
  // Call inputAutocompleteText but ensure we don't close the dropdown
  this.inputAutocompleteText(text, e);
  
  // Extra safeguard to ensure dropdown stays open
  setTimeout(() => {
    this.focused = true;
    this.inputBoxShow = true;
    if (this.$refs.input) {
    ...
    initializeMultiselectDropdown() {
      ...
      // Force open the dropdown with all options
      this.calAutocompleteList(true);
    ...
    // Modify the existing onCellFocus or similar method that initializes editing
    // to handle multiselect initial state
    

    onCellFocus() {
      // Your existing onCellFocus code
      
      // If it's a multiselect field, prepare the input box
      if (this.currentField && this.currentField.type === 'multiselect') {
        // Focus the input box
        if (this.$refs.input) {
          this.$refs.input.focus();
        ...
        // Make sure the dropdown shows all available options immediately
        this.calAutocompleteList(true);
      }
  ...
  //   console.log('Checkbox was clicked:', event.target.value); // Debugging output
  ...
  handleCheckboxClick(event) {
    ...
    // Update the input box value if needed
    this.inputBox.value = this.selectedItemsForAutocomplete.join(', ');

    // Optionally, force Vue to update the UI in case reactive changes are needed
    this.$forceUpdate();
    this.$nextTick(() => {
  ...
  const value = this.selectedItemsForAutocomplete.length > 0 
                ? this.selectedItemsForAutocomplete.join(', ') 
...
.checkbox-wrapper input[type="checkbox"] {
  pointer-events: none; /* Important: let the li handle the click */
}
